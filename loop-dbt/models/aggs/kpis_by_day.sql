...
, hack_loop_share_mrr_start as (
    select
        ...
        , SUM(CASE WHEN dim_members.membership_type = 'annual' then discount_mrr_offset END) AS start_discount_mrr_offset_annual
        , SUM(CASE WHEN dim_members.membership_type = 'monthly' then discount_mrr_offset END) AS start_discount_mrr_offset_monthly
        , SUM(CASE WHEN dim_members.membership_type = 'per-item' then discount_mrr_offset END) AS start_discount_mrr_offset_per_item
...
, hack_loop_share_mrr_end as (
    select
        ...
        , SUM(CASE WHEN dim_members.membership_type = 'annual' then discount_mrr_offset END) AS end_discount_mrr_offset_annual
        , SUM(CASE WHEN dim_members.membership_type = 'monthly' then discount_mrr_offset END) AS end_discount_mrr_offset_monthly
        , SUM(CASE WHEN dim_members.membership_type = 'per-item' then discount_mrr_offset END) AS end_discount_mrr_offset_per_item
    from {{ ref('loopshare_discount_hack') }}
    left join {{ ref('dim_members') }}
        using(member_id)
    group by 1
