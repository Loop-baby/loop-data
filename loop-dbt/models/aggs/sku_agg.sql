with inventory_count as (
	...
	left join (select bin, min(delivery_date) as first_del_date from {{ref('fact_loop')}} group by 1) fl
		using(bin)
	group by 1
),
active_count as (
	...
	from  {{ref('fact_daily_revenue')}}  fdr
...
from (select distinct sku, product_name from {{ref('dim_product')}} ) dp
