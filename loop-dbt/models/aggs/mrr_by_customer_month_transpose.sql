...
with base as (
    ...
    select
        ...
        , dim_members.last_name
        , dim_members.membership_type
        , month
        , case when dim_members.membership_type = 'annual' then 149.0/12 when dim_members.membership_type = 'monthly' then 18.0 else 0 end as membership_mrr
    ...
    where LEAST(next_month, CURRENT_DATE) > fact_loop.delivery_date
        AND LEAST(next_month, CURRENT_DATE) < NVL(fact_loop.pick_up_date, '9999-12-31')
    group by 1, 2, 3, 4, 5, 6
    order by dim_members.member_id, month
)
, expansions as (
    select *
