...
with fact_transaction as (
    select 
        ...
        , case
            when product_name ilike '%membership%'
                ...
                or (
                    ...
                    members_with_hacked_annual_membership.member_id is not null
                    AND fact_transaction.sales_type = 'return'
                    AND fact_transaction.quantity = 0
                    AND fact_transaction.product_name is null
                    AND fact_transaction.gross_sales >= 80
                )
            then true
            else false
        ...
        , row_number() over(
            partition by
                fact_transaction.order_id
                , fact_transaction.line_item_id
                , ABS(fact_transaction.quantity)
            order by transaction_ts desc
        ) as rn
    from {{ ref('fact_transaction') }}
    left join {{ ref('fact_line_item') }}
        using(line_item_id)
    left join {{ source('shopify', 'variants') }}
        on fact_line_item.variant_id = variants.id
