...
, first_order AS (
    ...
    from (
        ...
        select member_id,
            ...
            order_ts
        from 
            dw.fact_order --needs to change to archived_dw.fact_order
...
, base AS (
    select
        ...
        , case
            when mwahm.member_id is not null
                then 'annual'
            ...
            when cd.hs_membership_type ilike '%item%' or membership_product_name ilike '%item%'
                then 'per-item'
    ...
    left join {{ ref('membership_orders_rp') }} mo
    ...
    where COALESCE(mo2.is_blacklist_member, 0) != 1
        and cd.rn = 1
)
/*select
    base.member_id
...
select * from deliver_pickup_dates
