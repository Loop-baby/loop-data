with invoice_rn as (
    ...
    from {{ source('stripe', 'subscriptions') }} s inner join
        ...
        {{ ref('member_id_map_rp') }} mim on s.customer_id = mim.src_id and mim.src = 'stripe'
),
initial_order_map as (
    select bco.id as bigcommerce_order_id,
        ir.subscription_id
    from invoice_rn ir inner join 
        {{ ref('member_id_map_rp') }} mim on mim.member_id = ir.member_id left join
        {{ source('bigcommerce', 'orders') }} bco on mim.src_id = bco.customer_id and 
            mim.src = 'bigcommerce' 
