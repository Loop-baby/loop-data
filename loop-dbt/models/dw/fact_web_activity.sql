WITH base AS (
    select
        sessioned_events.*
        , coalesce(dm.member_id, dm2.member_id) as member_id
        -- precision here is day instead of exact time because our historical member start date wasn't recorded in a real system
        , case
            when  sessioned_events.timestamp::date > dateadd('d', 1, coalesce(dm.membership_start_dt, dm2.membership_start_dt)::date)
            then 'existing'
            else 'new'
        ...
        , first_value(referrer) over(partition by session_id order by sessioned_events.timestamp ROWS BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING) as session_referrer
        , max(case when url ilike '%gclid%' then 1 else 0 end) over(partition by session_id) as session_has_gclid
        , first_value(NULLIF(json_extract_path_text(f_parse_url_query_string(coalesce(url, '')), 'gclid'), '') ignore nulls) over(partition by session_id order by sessioned_events.timestamp ROWS BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING) as session_gcl_id
        , max(case when url ilike '%source=referral%' then 1 else 0 end) over(partition by session_id) as is_referral_session
        ...
        , max(case when url ilike '%loop-dev%' then 1 else 0 end) over(partition by session_id) as is_dev_session
        , sum(case when src_tbl = 'pages' then 1 else 0 end) over(partition by session_id rows between unbounded preceding and unbounded following) as session_page_views
        , CASE
            when regexp_replace(regexp_replace(sessioned_events.url::text, '.*products/'), '\\?.*') like '%/%' then null
            else regexp_replace(regexp_replace(sessioned_events.url::text, '.*products/'), '\\?.*')
        end as product_name
        , CASE
    ...
    left join {{ source('shopify', 'customers') }}
        on sessioned_events.master_user_id = customers.id
    ...
    left join {{ ref('dim_members') }} dm
        on dm.member_id = member_id_map.member_id
     left join {{ ref('dim_members') }} dm2
        on dm2.member_id = sessioned_events.master_user_id
),
campaigns as (
    select
        campaign_id
...
select base.*
	...
	, MAX(case when regexp_replace(REGEXP_REPLACE(url, '.*(pf_c_collection=)', ''),'(&).*', '') = '266791190699' and context_page_referrer = 'https://loop.baby/' then 1 else 0 end) OVER(partition by master_user_id order by base.timestamp ROWS BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING) as is_snoo_module
	, MAX(case when regexp_replace(REGEXP_REPLACE(url, '.*(pf_c_collection=)', ''),'(&).*', '') = '280972066987' and context_page_referrer = 'https://loop.baby/' then 1 else 0 end) OVER(partition by master_user_id order by base.timestamp ROWS BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING) as is_looptoyboxes_module
	, MAX(case when regexp_replace(REGEXP_REPLACE(url, '.*(pf_c_collection=)', ''),'(&).*', '') = '265671147691' and context_page_referrer = 'https://loop.baby/' then 1 else 0 end) OVER(partition by master_user_id order by base.timestamp ROWS BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING) as is_travel_module
	, MAX(case when regexp_replace(REGEXP_REPLACE(url, '.*(pf_c_collection=)', ''),'(&).*', '') = '265670918315' and context_page_referrer = 'https://loop.baby/' then 1 else 0 end) OVER(partition by master_user_id order by base.timestamp ROWS BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING) as is_0_3month_module
	, MAX(case when regexp_replace(REGEXP_REPLACE(url, '.*(pf_c_collection=)', ''),'(&).*', '') = '280295047339' and context_page_referrer = 'https://loop.baby/' then 1 else 0 end) OVER(partition by master_user_id order by base.timestamp ROWS BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING) as is_lovevery_module
	, MAX(case when regexp_replace(REGEXP_REPLACE(url, '.*(pf_c_collection=)', ''),'(&).*', '') = '266583212203' and context_page_referrer = 'https://loop.baby/' then 1 else 0 end) OVER(partition by master_user_id order by base.timestamp ROWS BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING) as is_babybjorn_module
	, MAX(case when regexp_replace(REGEXP_REPLACE(url, '.*(pf_c_collection=)', ''),'(&).*', '') = '266583179435' and context_page_referrer = 'https://loop.baby/' then 1 else 0 end) OVER(partition by master_user_id order by base.timestamp ROWS BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING) as is_stokke_module
	, MAX(case when regexp_replace(REGEXP_REPLACE(url, '.*(pf_c_collection=)', ''),'(&).*', '') = '280295211179' and context_page_referrer = 'https://loop.baby/' then 1 else 0 end) OVER(partition by master_user_id order by base.timestamp ROWS BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING) as is_topprod_module
    ...
    , coalesce(base.product_id, products.id) as product_id_master
    ...
    , split_part(first_value(url IGNORE NULLS) over(partition by base.session_id order by base.timestamp ROWS BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING), '?', 1) AS session_first_url
    , CASE 
...
left join {{source('shopify', 'products')}}
    on LOWER(trim(products.handle)) = lower(trim(base.product_name))
