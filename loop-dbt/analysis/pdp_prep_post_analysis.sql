WITH
 orders_completed as (
  SELECT session_id, fwa.timestamp, event, coalesce(fwa.member_id, member_id_map.member_id, fwa.anonymous_id, fwa.user_id) as user_identifier,
    row_number() OVER (PARTITION BY session_id ORDER BY timestamp) as order_rn
  FROM {{ref('fact_web_activity')}} as fwa
  LEFT JOIN {{ref('member_id_map')}} as member_id_map on member_id_map.src_id = anonymous_id and member_id_map.src = 'segment'
